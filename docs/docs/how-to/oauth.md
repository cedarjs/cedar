...
- **`unlink`** — removes a provider identity from the current user's account. Unlike the other three, this is a JSON `POST`, not a redirect round trip. It refuses to remove the last identity from an account that has no password set (`cannot_unlink_last_identity`) — otherwise the account would become permanently inaccessible. It also requires the request to come from a trusted `Origin` — the app's own origin by default, or one listed in `trustedOrigins`/`cors.origin` (see [`trustedOrigins`](../auth/dbauth.md#trustedorigins)) — as a CSRF defense; a request from an untrusted origin fails with `untrusted_origin`.
...
| `untrusted_origin`            | `unlink` flow, but the request's `Origin` header isn't trusted (CSRF defense — see [`trustedOrigins`](../auth/dbauth.md#trustedorigins)).                     |
