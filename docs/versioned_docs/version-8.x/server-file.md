...
Without the server file, to start the api side, you'd use binaries provided by `@cedarjs/api-server` such as `yarn cedar-server api` (you may also see this as `./node_modules/.bin/cedar-server api`).
...
- CMD [ "node_modules/.bin/cedar-server", "api" ]
