...
- **Deployment artifact** — `export default { fetch(request) }`, the
  WinterCG-compatible shape Cedar's build tooling emits for provider
  consumption.
...
WinterCG-compatible runtimes — app developers never write this directly:
...
- `CedarRequestContext` carries enrichments (parsed cookies, route
  ...
  not belong on a standard WinterCG `Request` object.
- The deployment artifact conforms to the WinterCG standard so Cedar
  outputs are consumable by Cloudflare Workers, Deno Deploy, Bun,
  Netlify Edge Functions, and any other WinterCG-compliant runtime.
...
1. Emit WinterCG-compatible deployment artifacts — modules that export
...
- `@universal-deploy/adapter-node` — wraps store entries with `srvx`
  (a WinterCG-compatible Node.js HTTP server) and `sirv` for static
...
- Cedar emits WinterCG-compatible `Fetchable` entries and registers
...
WinterCG-compatible `Fetchable` entries and wiring them into UD's
