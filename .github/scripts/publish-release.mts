 ...
 * its directory: one lockfile per package-manager overlay under the cjs and
 * esm overlay sets. They're generated by the release tooling against the
 * published packages and committed before tagging, so a tag run expects all
 * of them.
 */
const CREATE_CEDAR_APP_LOCKFILES = ['cjs', 'esm'].flatMap((overlaySet) => [
