...
// A packages-only run exists to put the packages on npm so that the release
// tooling can generate the lockfiles against them. A dry run of it would
// publish nothing and still report success, so the combination is refused
// rather than letting a manual dispatch (where dry-run defaults to true)
// look like the packages went out.
if (isDryRun && isPackagesOnly) {
  console.error(
    '❌ --packages-only and --dry-run cannot be combined. A dry run covers ' +
      'every package already; a packages-only run has to publish for real.',
  )
  process.exit(1)
}

/**
 * The lockfiles that ship inside this tree's create-cedar-app, relative to
 * its directory: one yarn.lock per base template, and one lockfile per
 * package-manager overlay under the cjs and esm overlay sets. They're
 * generated by the release tooling against the published packages and
 * committed before tagging, so a tag run expects all of them.
 */
const CREATE_CEDAR_APP_LOCKFILES = [
  'templates/ts/yarn.lock',
  'templates/js/yarn.lock',
  'templates/esm-ts/yarn.lock',
  'templates/esm-js/yarn.lock',
  ...['cjs', 'esm'].flatMap((overlaySet) => [
    `templates/overlays/${overlaySet}/yarn/yarn.lock`,
    `templates/overlays/${overlaySet}/npm/package-lock.json`,
    `templates/overlays/${overlaySet}/pnpm/pnpm-lock.yaml`,
  ]),
]
