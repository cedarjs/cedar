...
/**
 * @typedef {Object} PrHead
 * @property {string | undefined} branchName
 * @property {string | undefined} headSha
 *
 * @returns {Promise<PrHead>} the branch name and head sha for the current PR
 */
async function getPrHead() {
  ...
  return { branchName: json?.head?.ref, headSha: json?.head?.sha }
}
...
async function main() {
  ...
  const { branchName, headSha } = await getPrHead()
