...
describe('debugger sourcemaps (non-UD / Fastify)', () => {
  it('breakpoints hit correct lines in API functions with Fastify', async () => {
    killPort(DEBUG_PORT)
    ...
    try {
      ...
      expect(
        wsUrl,
        'Inspector WebSocket URL should be emitted (non-UD child process)',
      ).not.toBeNull()
      ...
      // Enable debugger (no runIfWaitingForDebugger — non-UD doesn't pause)
      await send('Debugger.enable')
      await send('Runtime.enable')

      // In non-UD mode, the handler script isn't loaded until the first
      // request reaches it. Fire one request to trigger compilation/loading.
      const warmup = fetch(
        `http://localhost:${WEB_PORT}/.api/functions/hello`,
      ).catch(() => {})

      // Wait for hello.ts to be parsed (Node.js reports the original .ts
      // URL via --enable-source-maps)
          ...
          const event = events.find(
            (e) =>
              ...
              (e.params.url === helloPath ||
                e.params.url.endsWith('/hello.ts')),
          )
      ...
      // Make fresh HTTP request to trigger the handler
