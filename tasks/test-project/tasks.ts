...
async function addModel(schema: string) {
  const path = `${fullPath('api/db/schema.prisma', { addExtension: false })}`
...
export async function apiTasks(
  ...
  const addDbAuth = async () => {
    ...
    const pathContactsSdl = `${outputPath}/api/src/graphql/contacts.sdl.ts`
    ...
    const pathPostsSdl = `${outputPath}/api/src/graphql/posts.sdl.ts`
    ...
    const pathRequireAuth = `${outputPath}/api/src/directives/requireAuth/requireAuth.test.ts`
    ...
    const pathSignupPageTs = `${outputPath}/web/src/pages/SignupPage/SignupPage.tsx`
    ...
    const pathAuthTs = `${outputPath}/api/src/functions/auth.ts`
  ...
  const addPrerender = async (): Promise<ListrTask[]> => {
    return [
      ...
      },
      {
        ...
        task: () => {
          const pathRoutes = `${outputPath}/web/src/Routes.tsx`
          ...
          const blogPostRouteHooksPath = `${outputPath}/web/src/pages/BlogPostPage/BlogPostPage.routeHooks.ts`
          ...
          const waterfallRouteHooksPath = `${outputPath}/web/src/pages/WaterfallPage/WaterfallPage.routeHooks.ts`
