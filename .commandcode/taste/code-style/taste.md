...
- Use `shell: process.platform === 'win32'` instead of unconditional `shell: true` for `spawnSync` calls. `shell: true` should only be used on Windows where corepack/PATH shims (e.g., `yarn.cmd`) need shell resolution; on Unix, native executables resolve fine without it. Confidence: 0.70
