...
- Do not use `as unknown as SomeType` to force a type cast — prefer proper type guards, narrowing, or union discrimination instead. If those options don't work, or produces too verbose code, use `// @ts-expect-error` with an explanatory comment instead. `as unknown as` bypasses the type system entirely and hides real type errors; `@ts-expect-error` is more honest about what's happening and easier to grep for. Confidence: 0.85
