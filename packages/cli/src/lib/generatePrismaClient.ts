...
/**
 * Check whether the Prisma client has already been generated by inspecting the
 * generated file content.
 */
export function isPrismaClientGenerated(fileContent: string): boolean {
  // The stub file generated by `@prisma/client` when the client hasn't been
  // generated yet always contains this message.
  const isStub = fileContent.includes('@prisma/client did not initialize yet.')

  if (isStub) {
    return false
  }

  // Prisma 7 (ESM output, `.mts`)
  if (fileContent.includes('export const PrismaClient')) {
    return true
  }

  // Prisma 6 / legacy output (CJS, `.js`)
  if (fileContent.includes('exports.Prisma.')) {
    return true
  }
...
export const generatePrismaClient = async ({
  ...
  if (!force) {
    ...
    if (isPrismaClientGenerated(prismaClientFile)) {
