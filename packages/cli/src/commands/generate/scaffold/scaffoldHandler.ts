...
const getIdType = (model: ScaffoldModel) => {
  return model.fields.find((field) => field.isId)?.type
}

const getIdName = (model: ScaffoldModel) => {
  return model.fields.find((field) => field.isId)?.name
}

const isAutoGeneratedColumnForScaffold = (column: ScaffoldField) => {
  const autoGeneratedFunctions = ['now', 'autoincrement']

  const columnDefaultFunction =
    typeof column.default === 'object' &&
    column.default !== null &&
    'name' in column.default
      ? (column.default as { name: string }).name
      : ''
...
export const shouldUseTailwindCSS = (flag: boolean | undefined) => {
  if (flag === undefined) {
...
export const files = async ({
  ...
  const model = (await getSchema(name)) as ScaffoldModel
...
const assetFiles = async (name: string, tailwind: boolean) => {
...
const modelRelatedVariables = (model: ScaffoldModel) => {
  type ValidationResult = string | false
  interface ComponentMetadataEntry {
    componentName?: string
    defaultProp?: string
    validation?: ((isRequired: boolean) => ValidationResult) | string
    listDisplayFunction?: string
    displayFunction?: string
    deserializeFunction?: string
  }

  const componentMetadata: Record<string, ComponentMetadataEntry> = {
    Enum: {
  ...
  const fieldsToImport = Object.keys(
    editableColumns.reduce<Record<string, true>>((accumulator, column) => {
      if (column.component) {
...
type TemplateStrings = ReturnType<typeof getTemplateStrings>
...
const pageFiles = async (
  ...
  const pluralName = pascalcase(pluralize(name))
  const singularName = pascalcase(singularize(name))
  const model = (await getSchema(singularName)) as ScaffoldModel
  const idType = getIdType(model)
  const idTsType = mapPrismaScalarToPagePropTsType(idType as string)
...
export const routes = async ({
  ...
  const nameVars = nameVariants(name)
  const model = (await getSchema(nameVars.singularPascalName)) as ScaffoldModel
...
// ListrTaskWrapper type alias for use in task callbacks
type AnyListrTask = ListrTaskWrapper<unknown, never, never>

const addHelperPackages = async (task: AnyListrTask) => {
...
export const splitPathAndModel = (pathSlashModel: string) => {
