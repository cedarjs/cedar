...
`,
  ...
  test('returns false when login is on a non-Route element', () => {
    vol.fromJSON(
      {
        'web/src/Routes.js': `
import { Router, Route } from '@cedarjs/router'
...
`,
  ...
  test('returns true even when the login route is nested inside PrivateSet', () => {
    // A login route wrapped in PrivateSet doesn't make sense (you'd need to
    // be logged in to reach the login page), so we don't guard against it --
    // we just detect the route by name.
    vol.fromJSON(
      {
        'web/src/Routes.js': `
...
`,
  ...
  test('prefers a login route over the landing page, even if the login route is (nonsensically) protected', () => {
    vol.fromJSON(
      {
        'web/src/auth.ts': 'export const { AuthProvider } = createAuth()',
        'web/src/Routes.js': `
