  ...
  const viteRscServer = await createViteServer({
    ...
    ssr: {
      ...
      optimizeDeps: {
        ...
        // the Browserify polyfill is because the old
        // `vite-plugin-node-polyfills` dependency added Browserify's
        // `node-util` to `node_modules`, so when Vite goes to resolve
        // `import { TextEncoder } from 'util` it'll find the one in
        ...
        // the node polyfill, so some kind of tree shaking is
