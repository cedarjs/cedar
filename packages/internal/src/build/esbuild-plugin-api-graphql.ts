...
export const cedarApiGraphqlPlugin = {
  ...
  setup(build: PluginBuild) {
    ...
    build.onLoad({ filter: /[/\\]graphql\.(ts|js)$/ }, async (args) => {
      ...
      // output: append .js to extensionless imports and rewrite explicit
      // .ts/.tsx extensions to .js. applyImportDir expands glob imports
      // (e.g. `src/directives/**/*.ts`) into individual bare specifiers
      // without extensions; without this step those imports fail at startup
      // with ERR_MODULE_NOT_FOUND on ESM projects, and explicit .ts imports
      // fail on both module formats.
