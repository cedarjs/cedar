...
// ---------------------------------------------------------------------------
// SDL file content generation
// ---------------------------------------------------------------------------

/**
 * Generate the full TypeScript source for the `__gqlorm__.sdl.ts` file.
 *
 * The generated file exports `schema` (a gql DocumentNode with type defs and
 * Query fields) and `resolvers` (an object with Query resolver functions).
 *
 * `gql` is globally available on the api side in Cedar projects, so no import
 * is needed for it.
 */
export function generateGqlormSdlContent(models: BackendModelInfo[]): string {
  if (models.length === 0) {
    return ''
  }
  ...
  for (const model of models) {
    ...
    // findUnique — singular with id argument
    if (model.idField) {
