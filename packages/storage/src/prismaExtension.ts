...
export const createUploadsExtension = <MNames extends ModelNames = ModelNames>(
  ...
  return PrismaExtension.defineExtension((client) => {
    // The `client` provided by defineExtension is the pre-extension client, so
    // calling it inside a query hook does not re-trigger the same hook and so
    // does not cause recursion.
    const queryExtends: Record<string, any> = {}
    ...
    for (const modelName in config) {
      ...
      queryExtends[modelName] = {
        ...
        async createMany({ query, args }: any) {
          try {
            const result = await query(args)
            return result
          } catch (e) {
        ...
        async update({ query, model, args }: any) {
          ...
          const uploadFieldsToUpdate = uploadFields.filter((field) => {
            return (
              args.data && typeof args.data === 'object' && field in args.data
            )
          })
          ...
          if (uploadFieldsToUpdate.length === 0) {
            ...
            const originalRecord = await (client as any)[
              model
            ].findFirstOrThrow({
              where: args.where,
        ...
        async updateMany({ query, model, args }: any) {
          // Check if any of the uploadFields are present in args.data
          // We only want to process fields that are being updated
          const uploadFieldsToUpdate = uploadFields.filter((field) => {
            return (
        ...
        async upsert({ query, model, args }: any) {
