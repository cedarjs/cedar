  ...
  if (fs.existsSync(path.join(templatesDir, dir, 'node_modules'))) {
    throw new Error(
      `Found node_modules in templates/${dir}. Remove it or the tests will ` +
        'time out while copying the template.',
    )
  }
}
...
describe('create-cedar-app', () => {
    ...
    expect(p.stdout).toMatchInlineSnapshot(`
      ...
      Options:
        ...
        -y, --yes                      Skip prompts and use defaults
            ...
            --node-check               Check if the installed version of Node is
                                       supported             [boolean] [default: true]
            --overwrite                Create even if target directory isn't empty
                                                            [boolean] [default: false]
            --typescript, --ts         Generate a TypeScript project
                                                             [boolean] [default: null]
            --git-init, --git          Initialize a git repository
                                                             [boolean] [default: null]
        -m, --commit-message           Commit message for the initial commit
                                                              [string] [default: null]
            --telemetry                Enables sending telemetry events for this
                                       create command and all Cedar CLI commands
                                       https://telemetry.redwoodjs.com
                                                             [boolean] [default: true]
            --install, --yarn-install  Install node modules. Skip via --no-install.
                                                             [boolean] [default: null]
  ...
  test('--yes, -y', async () => {
    // Running `yarn install` in the test times out and the subsequent step,
    ...
    // So we use `--no-install` to skip the install step.
